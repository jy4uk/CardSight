...
class TCGDataFetcher {
  delay = 0; // 0 second delay between requests to avoid rate limiting
  dbService;
  ...
  async fetchGroups(categoryIds = CATEGORY_IDS) {
    const allGroups = [];
  ...
  async fetchProducts(categoryId, groupId) {
  ...
  async fetchPrices(categoryId, groupId) {
  ...
  async fetchAllGroupData(categoryIds = CATEGORY_IDS) {
